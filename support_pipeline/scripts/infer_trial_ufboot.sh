...
cd $ufbootdir
iqtree -s $ctreefasta_with_refseq -B 1000 --bnni --prefix ufboot-output

out_file=$simdir"/ufboot-output.splits.nex"
...
conda activate hdag-benchmark
python $currdir/support_pipeline/inference.py save_supports -m "ufboot" -t $ctree -i $out_file -o $mrbayesdir/results.pkl
echo ""
echo ""
